...
from ghost import Blinky, Pinky, Clyde, Ghost
...
class Game:
    ...
    def reset_game(self):
        ...
        # Spawn ghosts in a small area at the top
        spawn_positions = [
        ...
        if len(valid_spawns) >= 1:
            self.ghosts.append(Blinky(self.maze, valid_spawns[0][0], valid_spawns[0][1]))
        if len(valid_spawns) >= 2:
            self.ghosts.append(Pinky(self.maze, valid_spawns[1][0], valid_spawns[1][1]))
        if len(valid_spawns) >= 3:
            self.ghosts.append(Clyde(self.maze, valid_spawns[2][0], valid_spawns[2][1]))
